...
def callback_analyseur_affiche(app):
    ...
    def deplay_image_result(button,url,drag_drop):
            ...
            for name_model in ['model_VGG16_transfert_learning_20','model_1500_10']:
                try:
                    ...
                    if url:
                        ...
                        list__img_df.append([model(app,name_model).select_model(url),url,name_model])

                    if drag_drop:
                        for img in drag_drop:
                            list__img_df.append([model(app,name_model).select_model(img),img,name_model])
        ...
        for couple in list__img_df:
            ...
            table = Pages(app).make_df_genre_to_html(couple)
